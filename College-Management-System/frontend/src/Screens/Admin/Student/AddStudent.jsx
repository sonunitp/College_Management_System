...
const AddStudent = () => {
  ...
  const getSubjectsData = (semester, branch) => {
    if (!semester || !branch) return;
  
    axios
      .get(`${baseApiURL()}/subject/getSubjectsBySemAndBranch`, {
  ...
  useEffect(() => {
    if (data.semester && data.branch) {
      getSubjectsData(data.semester, data.branch);
    }
  }, [data.semester, data.branch]);
